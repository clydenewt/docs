...
<Note>
  These specifications are for validator operations. If you only want to run a standard RPC node, the requirements are significantly lower. See [Running a Node](/operators/running-a-node) for lighter hardware requirements.
</Note>

<CardGroup cols={2}>
  <Card title="CPU" icon="microchip" iconType="duotone">
    <strong>Minimum:</strong> 16+ cores<br />
    <strong>Recommended:</strong> AMD Ryzen 9 7950X3D or EPYC 4584PX for optimal single-thread performance
  </Card>
...
<CardGroup cols={4}>
  ...
  <Card title="Oracle Infrastructure" icon="chart-line">
    Connect oracle sidecar for high-frequency price data. See [Oracle Documentation](/operators/oracle/overview) for setup details
  </Card>
...
<CardGroup cols={3}>
  ...
  <Card title="Oracle Setup" icon="chart-line" href="/operators/oracle/overview">
