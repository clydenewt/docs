...
After a one-month grace period following the Mercury upgrade, validators are subject to rotation criteria:

### Performance-Based Offboarding

A validator may be offboarded for:
- Receiving no rewards for two consecutive months
- Receiving no rewards for three months in a 6-month period
- Being tombstoned

### Downtime-Based Offboarding

A validator may be offboarded for:
- Failing to resume signing within 60 minutes of the first finalized block after an upgrade
- Failing to begin precommitting within 2 hours of upgrade instructions distribution

In both cases, validators are notified on first occurrence and marked for rotation on the second.
...
1. **Run Both Networks**: Maintain both mainnet and testnet nodes
2. **Initial Delegation**: Receive a small delegation from the DAO
3. **Probation Period**: Demonstrate adequate performance for three months
4. **Full Delegation**: Receive full delegation after successful probation

If performance is inadequate during probation, the DAO will redelegate to the next operator in line.
