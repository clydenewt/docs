...
To get started with CosmWasm development, make sure you have the following installed:

- [Rust](https://www.rust-lang.org/tools/install) (1.69.0+)
- [cargo-generate](https://github.com/cargo-generate/cargo-generate)
- [Docker](https://www.docker.com/) (for compilation)
...
To compile your contract into Wasm, you can use the provided script:
...
### Deploying on Neutron

You can deploy your contract to the Neutron network (version 6.0.1) using the `neutrond` CLI:

```bash
# Store your contract code on-chain
neutrond tx wasm store artifacts/my_first_contract.wasm \
  --from wallet \
  --chain-id neutron-1 \
...
neutrond tx wasm instantiate $CODE_ID '{"count": 100}' \
  --from wallet \
  --chain-id neutron-1 \
  --label "My Counter" \
  --gas-prices 0.025untrn \
...
neutrond query wasm contract-state smart $CONTRACT_ADDR '{"get_count": {}}' --output json
neutrond tx wasm execute $CONTRACT_ADDR '{"increment": {}}' \
  --from wallet \
  --chain-id neutron-1 \
  --gas-prices 0.025untrn \
  --gas auto \
...
This tutorial covered the basics of creating, compiling, and deploying a simple CosmWasm smart contract on Neutron. You've learned about contract state, entry points, messages, and how to interact with your deployed contract.

In the next tutorial, we'll explore how to use Neutron's unique Interchain Query (ICQ) module to build smart contracts that can access data from other blockchains. 