---
title: "ICQ Concepts"
description: "Detailed explanation of Interchain Queries architecture, verification mechanisms, and security model"
---
...
The security of the ICQ module relies on cryptographic verification rather than trusted third parties:
...
The ICQ system has several potential failure modes:

1. **Relayer Failures**: If no relayers submit results, queries become stale but no incorrect data is accepted
2. **Proof Verification Failures**: If proofs don't verify, results are rejected
3. **Remote Chain Reorgs**: If the remote chain experiences a reorganization, queries may temporarily return inconsistent results until the chain stabilizes
4. **Empty or Nil Values**: Due to a bug in ICS23, querying empty or nil values will fail verification

Robust contracts should handle these edge cases gracefully.
