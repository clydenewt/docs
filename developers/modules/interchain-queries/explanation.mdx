...
The amount required for the deposit is defined by the `query_deposit` [module parameter](/developers/modules/interchain-queries/reference#params) (default: **1,000,000 untrn** or **1 NTRN**).
...
The query creation deposit is refunded when an Interchain Query is removed by issuing a [RemoveInterchainQuery](/developers/modules/interchain-queries/reference#removeinterchainquery) message. This message can be sent either by the query owner or, under certain conditions, by anyone.
...
1. **`query_submit_timeout`**: A [registered query property](/developers/modules/interchain-queries/reference#registeredquery) specifying the number of blocks that define the renewable **query service period**. This period starts when a query is registered and renews with each query update. The `query_submit_timeout` value is set based on the [module parameters](/developers/modules/interchain-queries/reference#params) at the time of query registration.

2. **`last_submitted_result_local_height`**: A [registered query property](/developers/modules/interchain-queries/reference#registeredquery) indicating the block height on the home chain when the query was last updated.

3. **`registered_at_height`**: A [registered query property](/developers/modules/interchain-queries/reference#registeredquery) indicating the block height on the home chain when the query was initially registered.  
...
Instead of saving results on-chain, the `interchainqueries` module directly sends the full TX results to the owning smart contracts during the [TxQueryResult](/developers/modules/interchain-queries/reference#messagetxqueryresult) `sudo` call. Once the `sudo` call is successfully completed, the results are discarded, minimizing storage use.
...
The `interchainqueries` module relies on the [contractmanager](/developers/modules/contractmanager/overview) module to handle `sudo` operations. The `contractmanager` module [enforces strict gas limits](/developers/modules/contractmanager/overview#gas-limitation) on `sudo` callbacks, defined by its `sudo_call_gas_limit` parameter, to prevent excessive gas usage.  
...
If a `sudo` callback fails, the `interchainqueries` module records the failure in the [contractmanager](/developers/modules/contractmanager/overview) module. This record includes all relevant information about the query and a redacted error message (reduced to the codespace and error code). A full error message is emitted as an [event](https://docs.cosmos.network/v0.50/learn/advanced/events) to provide more context about the failure. For details, see the [message events emission](/developers/modules/interchain-queries/reference#sudo) section.

Failures due to `out of gas` errors are also recorded.  

Failed query result submissions can be retrieved from the `contractmanager` module and resubmitted to the `interchainqueries` module. For more information about reading and resubmitting failed operations, refer to the [contractmanager documentation](/developers/modules/contractmanager/overview).

## How are Interchain Query results removed?

When a [RemoveInterchainQuery](/developers/modules/interchain-queries/reference#removeinterchainquery) message is successfully executed, the results of the respective Interchain Query are removed from the chain's storage. The removal process depends on the type of the Interchain Query:
