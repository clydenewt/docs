...
- `amount-in`: Amount of input tokens
- `exit-limit-price`: Minimum allowable price for `amount_out/amount_in`. If the route price is less than this exit limit price, the route will fail and the next route (if provided) will be tried. Formula: `exit-limit-price = minimum_output_amount / input_amount`
