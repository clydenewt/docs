...
Contracts can resubmit their failures for processing. This feature allows:

1. Contracts to retry processing of a failed sudo call after fixing bugs or optimizing gas usage.
2. Developers to update contract logic and then process previously failed acknowledgments.

<Warning>
Resubmission uses the same gas limits as the original sudo call. If your failure was due to exceeding the sudo gas limit, you must optimize your contract's sudo handler to use less gas, or request a governance proposal to increase the `sudo_call_gas_limit` parameter.
</Warning>
...
flowchart TD
    ...
    I --> J[Retry Sudo<br>Same Gas Limit]
```
