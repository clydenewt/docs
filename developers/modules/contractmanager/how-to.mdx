...
2. Resubmission still runs with the same sudo gas limit constraints as the original call
3. You should fix any issues in your contract before resubmitting (e.g., optimize gas usage or simplify logic)

<Warning>
If your contract failed due to running out of gas within the sudo gas limit, simply resubmitting will likely fail again. You must either optimize your contract's sudo handler to use less gas, or the sudo gas limit parameter must be increased through governance.
</Warning>
