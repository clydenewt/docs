...
1. **EXECUTION_STAGE_BEGIN_BLOCKER** (value: 1): Schedules are executed at the beginning of the block, before any transactions. This is useful for operations that need to happen before other transactions in the block.

2. **EXECUTION_STAGE_END_BLOCKER** (value: 0, default): Schedules are executed at the end of the block, after all transactions. This is useful for operations that should occur after all other transactions in the block have been processed.
...
<Info>
Each execution stage is processed independently during its respective blocker phase. The `limit` parameter applies separately to each stage, meaning up to `limit` schedules can execute in BeginBlock and up to `limit` schedules can execute in EndBlock within the same block.
</Info>
...
- There's a network-wide `limit` parameter (default: 5) that restricts the number of schedules processed per execution stage in a single block
- If more schedules are due than the limit allows, some are delayed until subsequent blocks
- Schedules are processed in KVStore iteration order (typically alphabetical by schedule name)
...
When multiple schedules are due for execution in the same block, they are processed in the order they appear during KVStore iteration. Since schedule names are used as keys in the store, schedules are typically processed in alphabetical order by name.

<Note>
The execution order is determined by the KVStore iteration order, not by `last_execution_height`. If multiple schedules are ready for execution and the limit is reached, later schedules (alphabetically) will be deferred to the next block where they become eligible.
</Note>
...
|-----------|-------------|---------|-------------|
| `security_address` | Address authorized for security operations | `""` (empty) | Must be valid bech32 address or empty |
| `limit` | Maximum schedules executed per execution stage per block | `5` | Must be greater than 0 |

<Note>
The `limit` parameter applies separately to each execution stage (BEGIN_BLOCKER and END_BLOCKER), meaning up to `limit` schedules can execute during BeginBlock and up to `limit` schedules during EndBlock in the same block.
</Note>
