---
title: "Cron Concepts"
description: "Key concepts and principles of the Cron module"
---

# Cron Module: Core Concepts

This document explains the fundamental concepts and architecture of the Cron module in Neutron.

## On-Chain Automation

Traditional blockchain applications require external triggers for time-sensitive operations. This creates several challenges:
...
The execution model of the Cron module follows these principles:

### Trigger Mechanism

Schedules are triggered based on block height comparisons:
```
current_block_height >= (last_execute_height + period)
```

When this condition is met during a block's `EndBlock` phase, the module executes the schedule's messages.

### Message Execution

Messages in a schedule are executed sequentially. Each message is:

1. **Validated**: Checked for correct formatting and valid targets
2. **Executed**: Processed by its respective handler
3. **Logged**: Results are recorded for debugging and monitoring

### Gas Management

The Cron module implements a comprehensive gas management system:

- **Per-Message Limits**: Each message specifies its own gas limit
- **Block-wide Cap**: Total gas used by all scheduled tasks in a block is capped
- **Prioritization**: When gas demands exceed the cap, schedules are prioritized based on last execution time

## Schedule Lifecycle

A schedule goes through several stages during its lifecycle:

1. **Creation**: A smart contract creates a schedule with a unique name
2. **Registration**: The module stores the schedule in its state
3. **Execution**: At appropriate block heights, the schedule's messages are executed
4. **Update**: The schedule's owner can modify its parameters if needed
5. **Removal**: When no longer needed, the schedule can be removed by its owner
...
The Cron module was designed according to these core principles:

### Determinism

All schedule executions are deterministic, ensuring:
- The same schedules execute at the same block heights across all nodes
- Execution order is consistent and predictable
- Results are identical across the entire network

### Efficiency

The module optimizes resource usage through:
- Efficient state storage patterns
- Batched execution processing
- Gas usage constraints

### Security

Security is ensured through:
- Strict ownership checks for schedule management
- Gas limitations to prevent DoS attacks
- Execution isolation to prevent cross-schedule interference

### Flexibility

The module is designed to be flexible, supporting:
- Various message types for different use cases
- Single or multiple message executions per schedule
