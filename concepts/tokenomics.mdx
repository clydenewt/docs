...
The Neutron token (NTRN) powers the economic model of the Neutron blockchain, enabling the secure operation of the network while incentivizing participation in the ecosystem.

## Token Supply and Inflation

Unlike many proof-of-stake networks, Neutron maintains a **fixed token supply** with zero inflation. 

- **Total Supply**: 1,000,000,000 NTRN
- **Inflation Rate**: 0% (fixed supply)
- **Block Rewards**: None (rewards come from the DAO Treasury)

This deflationary model ensures that NTRN token value isn't diluted over time through continuous issuance, promoting long-term value preservation.
